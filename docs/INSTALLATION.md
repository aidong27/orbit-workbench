...
本指南适用于星轨工作台 `0.2.0-alpha.1`：Windows x64 与 macOS arm64。
...
```text
Orbit-Workbench-0.2.0-alpha.1-Windows-x64-Setup.exe
```
...
```text
Orbit-Workbench-0.2.0-alpha.1-Windows-x64-Portable.exe
```
...
```text
Orbit-Workbench-0.2.0-alpha.1-macOS-arm64.dmg
Orbit-Workbench-0.2.0-alpha.1-macOS-arm64.zip
```
...
```powershell
Get-FileHash '.\Orbit-Workbench-0.2.0-alpha.1-Windows-x64-Setup.exe' -Algorithm SHA256
```

Windows CMD：

```bat
certutil -hashfile Orbit-Workbench-0.2.0-alpha.1-Windows-x64-Setup.exe SHA256
```

macOS：

```bash
shasum -a 256 Orbit-Workbench-0.2.0-alpha.1-macOS-arm64.dmg
```
...
星轨工作台使用 Electron 的用户数据目录保存界面偏好和裁剪后的时间线。删除前先退出应用，并确认不再需要本地会话标题与文本。由于不同系统目录不同，项目不会在文档中提供宽泛的递归删除命令；可以通过系统应用数据管理或开发者工具定位准确目录。
