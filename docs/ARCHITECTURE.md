...
客户端使用带 `schemaVersion` 与保存时间的 v4 信封，把有界的会话草稿、工作区路径、最近一次 Git 分支/文件状态/差异统计快照、界面偏好、会话标题和裁剪后的时间线写入 Electron 本机存储；当前不加密。空工作区是合法状态；加载时经 Zod 校验、v3/v2/v1 事务式迁移和引用修复，只有 v4 成功写入后才清理旧键。最多保留 40 个会话、每个会话 160 条时间线，并对单条消息、草稿总量和单会话持久化体积设置上限。工具原始输入、输出和中间载荷不会持久化。ACP Session ID、已确认模式、可用模式和权限队列只属于当前进程。
