...
The current release is **`0.2.0-alpha.1`**. Alpha builds are intended for testing and review, not irreplaceable workspaces.
...
Orbit Workbench's UI and local state do not read or store `XAI_API_KEY`. The launched Grok child process inherits the app's startup environment so the CLI can apply its own authentication rules. The sandboxed renderer has no Node.js access; validated IPC calls cross into the Electron main process, which connects to the user's CLI over ACP/stdio. Only trimmed UI history is persisted locally.
