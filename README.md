...
当前版本为 **`0.2.0-alpha.2`**。Alpha 构建用于测试和审阅，不建议用于无法回滚的重要项目。
...
- Windows x64：`Orbit-Workbench-0.2.0-alpha.2-Windows-x64-Setup.exe`
- Windows x64 便携版：`Orbit-Workbench-0.2.0-alpha.2-Windows-x64-Portable.exe`
- macOS arm64：`Orbit-Workbench-0.2.0-alpha.2-macOS-arm64.dmg` 或 `.zip`
...
- 本地保存界面偏好、工作区路径、会话标题、裁剪后的时间线，以及最近一次 Git 分支、文件状态列表和差异统计快照；这些数据使用带版本号和校验的 v2 格式，但当前不加密。
...
进一步阅读：[架构说明](docs/ARCHITECTURE.md) · [0.2.0-alpha.2 发布说明](docs/RELEASE_NOTES.md) · [发布检查清单](docs/REVIEW_CHECKLIST.md) · [变更记录](CHANGELOG.md)
