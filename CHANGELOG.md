...
- 连接中心加入带二次确认的 Grok 账号退出与切换入口；优先使用 ACP logout 能力，并由绝对路径官方 CLI 有界复核。
- 设置加入标准/大字可读性选项，默认使用大字；任务输入框会在 52–220px 间自动增高。
- 命令面板可以搜索并直接切换已有工作区和最近任务。

### Changed

- “重新检测”现在会完整停止旧 ACP 进程树后重新启动、重新初始化和重新认证，不再复用可能持有旧账号的 ready 连接。
- 会话按最近活动时间排序和裁剪；Windows 最小窗口约束降低到 560×420 DIP，改善 Snap 与高缩放窗口可用性。
- 本地状态升级为 v4，按会话持久化有界草稿、自动连接偏好与文字大小，并事务式迁移 v3/v2/v1；旧版升级默认保持未连接且不猜测账号状态。

### Fixed

- 明确退出后关闭自动连接，避免应用重启时静默恢复另一个缓存账号。
- 强制重连会使旧 ACP Session ID 失效，并把已有记录诚实标记为仅本地历史；历史里的未发送草稿可以查看并移入新任务。
- 无 `XAI_API_KEY` 时不再尝试 API Key 认证并覆盖真正的未登录错误。
- 权限请求过期后立即禁用所有决定按钮；完整工具输入改为键盘可读的只读区域。
- 断线后清除旧 Agent 元数据，空任务页面不再重复显示多块相同连接告警；连接完成后的引导标题和 ACP 文案不再误报“尚未连接”或“认证完成”。
- 模式已被代理确认后，迟到的调用超时不会把真实成功状态覆盖为失败；中文输入法组合期间切换会话会重建输入控件。
- 输入框会在窗口 Snap、边栏或检查器改变宽度时重新测量，不再裁掉因换行增高的草稿。
- 后发的断开或退出账号操作会取消仍在进行的重连，窗口关闭或注销结束后不会被旧请求重新拉起代理。

### Security

- 注销流程不读取或删除 `auth.json`，CLI 非零退出一律不能证明注销成功；账号标签只来自官方 CLI 的脱敏输出且不进入持久化或诊断，复制诊断会额外隐藏邮箱。
- 强制重连只有在旧进程树确认结束后才允许启动替代进程，避免两个账号上下文并存。
...
## 0.2.0-alpha.4 开发快照（未公开发布）- 2026-07-25
...
## 0.2.0-alpha.3 开发快照（未公开发布）- 2026-07-25
...
## 0.2.0-alpha.2 开发快照（未公开发布）- 2026-07-21
